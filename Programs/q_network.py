...
def PI_network(hidden_dim=10, n_actions=14):
    hand = Input((13,4))  # zero everywhere and 1 for your cards
    board = Input((3,13,4))  # 3 rounds of board: flop [0,:,:], turn [1,:,:] and river [2,:,:]
    pot = Input((1,))
    stack = Input((1,))
    ...
    situation_with_opponent = Dense(hidden_dim, activation='selu')(Concatenate()([plays, pbds, bh]))
    situation_with_opponent = Dense(hidden_dim, activation='selu')(situation_with_opponent)
    probabilities_of_actions = Dense(n_actions, activation='softmax')(situation_with_opponent)

    PI = Model([hand, board, pot, stack, opponent_stack, blinds, dealer, preflop_plays, flop_plays, turn_plays, river_plays], [probabilities_of_actions, probabilities_of_each_combination_board_only, probabilities_of_each_combination_board_hand, board_value, board_hand_value])
