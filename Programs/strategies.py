...
def strategy_RL_aux(player, board, pot, actions, b_round, opponent_stack, opponent_side_pot, Q, greedy=True, blinds=BLINDS, verbose=False):
    ...
    if greedy:
        best_possible_action_bucket = np.argmax([Q_value for k, Q_value in enumerate(Q_values) if idx_to_bucket(k) in possible_actions])
        ...
        action = bucket_to_action(best_possible_action_bucket, actions, b_round, player, opponent_side_pot, blinds=blinds)
    else:
