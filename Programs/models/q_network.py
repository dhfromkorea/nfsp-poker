...
class SharedNetwork(t.nn.Module):
    ...
    def forward(self, hand, board, pot, stack, opponent_stack, big_blind, dealer, preflop_plays, flop_plays, turn_plays, river_plays):
        ...
        turn_alone = selu(self.fc8(flop_alone))
        river_alone = selu(self.fc9(flatten(board[:, 2, :, :])))
        river_alone = selu(self.fc10(flop_alone))
        ...
        processed_flop = selu(self.fc20(t.cat([flatten(flop_plays), flop_alone], -1)))
        processed_turn = selu(self.fc21(t.cat([flatten(turn_plays), turn_alone], -1)))
        processed_river = selu(self.fc22(t.cat([flatten(river_plays), river_alone], -1)))
        ...
        situation_with_opponent = selu(self.fc25(t.cat([plays, pbds, bh], -1)))
...
class QNetwork(t.nn.Module):
    def __init__(self, n_actions, hidden_dim, shared_network=None, pi_network=None,
        ...
        for i in range(1, 6):
            setattr(self, 'conv' + str(i), getattr(self.shared_network, 'conv' + str(i)))
        for i in range(1, 27):
...
class PiNetwork(t.nn.Module):
    ...
    def forward(self, hand, board, pot, stack, opponent_stack, big_blind, dealer, preflop_plays, flop_plays, turn_plays, river_plays):
        ...
        return pi_values, hand_strength, probabilities_of_each_combination_board_hand, probabilities_of_each_combination_board_only
